...
from isegm.model.modeling.gatlayers import GATFusionBlockPosOnly, GATFusionBlock
from isegm.model.modeling.gcnlayers import GCNFusionBlockPosOnly, GCNFusionBlock
...
class Block(nn.Module):
    ...
    def forward(self, x, points, im_sz):
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=(224,224), patch_size=(16, 16), in_chans=3, num_classes=1000, embed_dim=768,
                 ...
                 proj_drop_rate=0., norm_layer=None, act_layer=None, cls_feature_dim=None, global_pool=False, gnn='gat'):
        super().__init__()
        ...
        if self.gnn=='gat':
            self.gnn_fusion1 = GATFusionBlock(embed_dim, norm_layer=norm_layer)
            self.gnn_fusion2 = GATFusionBlock(embed_dim, norm_layer=norm_layer)
            self.fusion = nn.Linear(2*embed_dim, embed_dim)
            self.gnn_norm = norm_layer(2*embed_dim)
            self.gamma = torch.nn.Parameter(torch.randn(1, 1, embed_dim), requires_grad=True)
        elif self.gnn=='gcn':
            self.gnn_fusion1 = GCNFusionBlock(embed_dim, norm_layer=norm_layer)
            self.gnn_fusion2 = GCNFusionBlock(embed_dim, norm_layer=norm_layer)
            self.fusion = nn.Linear(2*embed_dim, embed_dim)
            self.gnn_norm = norm_layer(2*embed_dim)
            self.gamma = torch.nn.Parameter(torch.randn(1, 1, embed_dim), requires_grad=True)
        elif self.gnn=='gatpos':
            self.gnn_fusion1 = GATFusionBlockPosOnly(embed_dim, norm_layer=norm_layer)
            self.gnn_fusion2 = GATFusionBlockPosOnly(embed_dim, norm_layer=norm_layer)
            self.fusion = nn.Linear(2*embed_dim, embed_dim)
            self.gnn_norm = norm_layer(2*embed_dim)
            self.gamma = torch.nn.Parameter(torch.randn(1, 1, embed_dim), requires_grad=True)
        elif self.gnn=='gcnpos':
            self.gnn_fusion1 = GCNFusionBlockPosOnly(embed_dim, norm_layer=norm_layer)
            self.gnn_fusion2 = GCNFusionBlockPosOnly(embed_dim, norm_layer=norm_layer)
            self.fusion = nn.Linear(2*embed_dim, embed_dim)
            self.gnn_norm = norm_layer(2*embed_dim)
            self.gamma = torch.nn.Parameter(torch.randn(1, 1, embed_dim), requires_grad=True)
    ...
    def forward_backbone(self, x, additional_features=None, shuffle=False, points=None):
        idxs = self.pt2patchidx(points, x.shape[2], patch_sz=16)
        f_len = (x.shape[2] / 16) ** 2
        B, n_p = idxs.shape
        mask = []
        for b_z in range(B):
            idx_neg = idxs[b_z, n_p // 2:][idxs[b_z, n_p // 2:] >= 0].contiguous().type(torch.long)
            idx_pos = idxs[b_z, : n_p // 2][idxs[b_z, : n_p // 2] >= 0].contiguous().type(torch.long)
            tf = torch.zeros([1, int(f_len)]).type_as(x)
            # print("形状：", tf.shape, "正索引：", idx_pos, "负索引： ", idx_neg)
            tf[:, idx_neg] = -1
            tf[:, idx_pos] = 1
        ...
        if shuffle:
            ...
            for i in range(1, num_blocks + 1):
                if i % num_blocks_per_group:
                    ...
                    x = self.unpatchify(x)
                    is_patchified = False
                x = self.blocks[i-1](x, points, x.shape[2])
                if i == 3:
                    early_x = self.unpatchify(x)
        if self.gnn is not None:
            B = x.shape[0]
            early_x, e_gnn = self.gnn_fusion1(early_x, mask, self.pos_embed[:, 1:])
            x_, gnn = self.gnn_fusion2(x, mask, self.pos_embed[:, 1:])
            ddim = 1536 if num_blocks == 12 else 2048
            x = x + self.gamma * self.fusion(self.gnn_norm(torch.cat([early_x, x_], dim=2).view(-1, ddim))).view(B, -1, ddim//2)
        return x, [e_gnn, gnn]
