...
from .models import PatchEmbed, Attention, Mlp
...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., mlp_drop=0., qkv_bias=False, attn_drop=0., 
        proj_drop=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, use_gcn=False):
        super().__init__()
    ...
    def forward(self, x, points):
        if self.use_gcn:
            scale = np.sqrt(x.shape[1]) // 14
            idxs = self.pt2patchidx(points, im_sz=224*scale, patch_sz=16)
            f_len = x.shape[1]
            B, n_p = idxs.shape
            mask = []
            for b_z in range(B):
                ...
                tf[:, idx_pos] = 1
                mask.append(tf)
            x, _ = self.gcn_fusion(x, torch.vstack(mask))
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=(224,224), patch_size=(16, 16), in_chans=3, num_classes=1000, embed_dim=768, 
                 ...
                 proj_drop_rate=0., norm_layer=None, act_layer=None, cls_feature_dim=None, global_pool=False, use_gcn=False):
        super().__init__()
        ...
        self.blocks = nn.Sequential(*[
            Block(dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, 
              ...
              act_layer=act_layer, use_gcn=self.use_gcn)
