...
class Clicker(object):
    ...
    def _get_next_click(self, pred_mask, padding=True):
        ...
        # fn_max_dist = np.random.choice(list(torch.from_numpy(fn_mask_dt).view(-1).topk(50)[0].numpy()))
        # fp_max_dist = np.random.choice(list(torch.from_numpy(fp_mask_dt).view(-1).topk(50)[0].numpy()))
