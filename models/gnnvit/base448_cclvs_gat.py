from isegm.data.datasets.hflicker import HFlickerDataset
from isegm.data.datasets.hqseg44k import HQSegDataset
from isegm.data.datasets.saliency import SaliencyDataset
from isegm.data.datasets.thinobject import ThinObjectDataset
from isegm.data.datasets.ytb_vos import YouTubeDataset
from isegm.model.is_plainvit_model import PlainVitModel
from isegm.utils.exp_imports.default import *
from isegm.model.modeling.transformer_helper.cross_entropy_loss import CrossEntropyLoss
from torch.nn.parallel import DistributedDataParallel as DDP

MODEL_NAME = 'gatvit_base448'
...
def init_model(cfg, rank=0):
    ...
    if cfg.distributed:
        model = DDP(model.to(rank), find_unused_parameters=False)
    else:
        model = model.to(rank)
...
def train(model, cfg, model_cfg, rank=0):
    ...
    # loss_cfg.sim_loss = SimLoss()
    # loss_cfg.sim_loss_weight = 1.0
    ...
    trainer = ISTrainer(model, cfg, model_cfg, loss_cfg,
                        ...
                        rank=rank)
    trainer.run(num_epochs=1000, validation=False)
