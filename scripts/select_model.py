...
from scripts.evaluate_model import calculate_binary_cigr
...
from isegm.inference.evaluation import evaluate_dataset, evaluate_dataset_mp
...
def main():
    ...
    while True:
        time.sleep(1)
        prefix = args.exp_path + '/checkpoints/'
        ckpts = os.listdir(prefix)
        ckpts.sort(key=lambda x: int(x[:-4]))
        new_len = len(ckpts)
        if new_len != old_len:
            time.sleep(3)
            args.checkpoint = prefix + '{}'.format(ckpts[-1])
            old_len = new_len
        else:
            continue
        ...
        print_header = True # single_model_eval
        for checkpoint_path in [checkpoints_list[0],]:
            logs_prefix = checkpoint_path.stem
            ...
            for dataset_name in args.datasets.split(','):
                ...
                dataset_results = evaluate_dataset_mp(dataset, predictor, pred_thr=args.thresh,
...
def get_checkpoints_list_and_logs_path(args, cfg):
    ...
    if args.exp_path:
        ...
        try:
            ...
            exp_path_prefix = Path(args.exp_path)
            candidates = list(exp_path_prefix.parent.glob(exp_path_prefix.stem + '*'))
            assert len(candidates) == 1, "Invalid experiment path."
        ...
        logs_path = args.logs_path / args.checkpoint.split('/')[-5] / args.checkpoint.split('/')[-4] # exp_path.relative_to(cfg.EXPS_PATH)
    else:
...
def save_results(args, row_name, dataset_name, logs_path, logs_prefix, dataset_results,
    ...
    cigr = 0.0
    for ioulist in all_ious:
        cigr += calculate_binary_cigr(ioulist)
    cigr = cigr / len(all_ious)
    ...
    header, table_row = utils.get_results_table(noc_list, over_max_list, row_name, dataset_name,
                                                mean_spc, elapsed_time, cigr, args.n_clicks,
