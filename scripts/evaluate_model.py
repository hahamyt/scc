...
def main():
    ...
    checkpoints_list = sorted(checkpoints_list)
...
def calculate_binary_cigr(iou_values):
    if len(iou_values) == 1:
        return 0.0
    iou_values = np.array(iou_values)
    growth_rates = (iou_values[1:] - iou_values[:-1]) / iou_values[:-1]
    # return np.mean(growth_rates) #  if len(growth_rates) != 0 else 0.0
    binary_growth_rates = np.where(growth_rates > 0, 1, 0)
    binary_cigr = np.sum(binary_growth_rates) / (len(iou_values) - 1)
    return binary_cigr
